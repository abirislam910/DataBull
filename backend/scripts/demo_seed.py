...
tok = call("/auth/signup", {"email": "operator@example.com", "password": "a-good-password"})["access_token"]
print("signed up operator@example.com")

specs = [("Pump-3","flow","L/min",50,10.0,80.0), ("Furnace-1","temperature","°C",25,5.0,40.0), ("Line-Press","pressure","kPa",100,None,None)]
now = datetime.now(timezone.utc)
random.seed(7)
for name, dtype, unit, baseline, lo, hi in specs:
    dev = call("/devices", {"name":name,"type":dtype,"unit":unit,"min_threshold":lo,"max_threshold":hi}, tok)
    rows=[]
